...
public class ChargeSettingServiceImpl extends ServiceImpl<ChargeSettingDao, ChargeSetting> implements ChargeSettingService {
    ...
    public boolean add(ChargeSettingAddDto dto) {
        ...
        int count = selectCount(new EntityWrapper<ChargeSetting>().eq("sys_user_id",sysUser.getId()).eq("money", dto.getMoney()));
        if (count > 0) {
            LeaseException.throwSystemException(LeaseExceEnums.CHARGE_SETTING_EXIST);
        }
        count = selectCount(new EntityWrapper<ChargeSetting>().eq("sys_user_id",sysUser.getId()));
    ...
    public List<ChargeSettingListDto> list(ChargeSettingQueryDto query) {
        List<Integer> userIds = sysUserService.resolveSysUserAllSubIds(sysUserService.getCurrentUserOwner());
        query.setAccessableUserIds(userIds);
