...
public class OrderController extends BaseController {
    ...
    @ApiOperation(value = "用户端创建钱包充值订单", notes = "此接口主要用户端下充值单")
    @ApiImplicitParam(paramType = "header", name = Constants.TOKEN_HEADER_NAME)
    ...
    @ApiOperation(value = "用户端创建押金钱包订单", notes = "此接口主要用户端下押金单")
    @ApiImplicitParam(paramType = "header", name = Constants.TOKEN_HEADER_NAME)
    @RequestMapping(value = "/deposit/order", method = RequestMethod.POST)
    public
    @ResponseBody
    ResponseObject depositOrder(@RequestBody @Valid RequestObject requestObject) {
        DepositOrderDto depositOrderDto = new DepositOrderDto();
        UserWalletChargeOrder userWalletChargeOrder = userWalletChargeOrderService.createDepositOrder(depositOrderDto);
        return success(userWalletChargeOrder);
    }
    ...
    @ApiOperation(value = "用户端查询使用中订单列表", notes = "查询当前openid使用中的订单")
    @ApiImplicitParam(paramType = "header", name = Constants.TOKEN_HEADER_NAME)
    @RequestMapping(value = "/using/order/list", method = RequestMethod.POST)
    public
    @ResponseBody
    ResponseObject<Page<AppOrderVo>> getUsingOrderList(@RequestBody @Valid RequestObject<Pageable<AppOrderListDto>> requestObject) {
        Page<AppOrderVo> order = orderBaseService.getForAppOrderList(requestObject.getData());
        return success(order);
    }
