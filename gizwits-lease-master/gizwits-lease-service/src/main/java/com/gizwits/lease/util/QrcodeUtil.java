...
//        url += "/app/wx/weixin?deviceId=" + sno;
    ...
    public static Map<String, String> createAndSaveQrcode(Device device, Product product, SysUserExt sysUserExt) {
        ...
        String qrcodeHost;
        if(sysUserExt != null && org.apache.commons.lang.StringUtils.isNotBlank(sysUserExt.getQrcodeHost())){
            qrcodeHost = sysUserExt.getQrcodeHost();
        }else{
            qrcodeHost = SysConfigUtils.get(CommonSystemConfig.class).getHostWithContext();
        }
        String contentUrl = getQrcodeContent(device.getSno(), qrcodeHost);
        //如果是WEB生成二维码
    ...
    private static Map<String, String> createQrcodeForWeb(String title, String contentUrl, String sno) {
        ...
        try {
            ...
            Font font = new Font("HelveticaNeue", Font.BOLD, 27);
            gs.setFont(font);
            ...
            //在这里写mac
            gs.drawString(title, x, 470);
            gs.dispose();
            bufImg.flush();
            String fileName = SysConfigUtils.get(CommonSystemConfig.class).getQrcodePath() + "/" + sno + ".jpg";
            logger.debug("二维码创建在"+fileName);
...
//                return QrTicket.fromJson(res);
    ...
    public static void  main(String[] args){
        String titie = "sno: 12345678900987654321";
        String contentUrl = "http://app.itosdk.com";
        String sno = "12345678900987654321";
        createQrcodeForWeb(titie,contentUrl,sno);
    }
}