...
public class DeviceOnlineHandler implements PushEventHandler {
    ...
    public void processPushEventMessage(JSONObject json) {
        ...
        log.info("设备上线===>pk:{} did:{} mac:{} ip:{} city:{}", productKey, did, mac, ip, city);
        Device onlineDevice = deviceService.getDeviceByMacAndPk(mac, productKey);
        if (onlineDevice == null) {
            log.info("====> 设备在系统中不存在,mac[" + mac + "],productKey[" + productKey + "]需要增加到数据库");
            return;
        }

        //检查设备是否有要下发的指令
        if (redisService.containsDeviceControlCommand(productKey, mac)) {
            if (DeviceControlAPI.remoteControl(productKey, onlineDevice.getGizDid(), redisService.getDeviceControlCommand(productKey, mac))) {
        ...
        deviceService.updateDeviceOnOffLineStatus(mac, productKey, did, true);
        redisService.cacheDeviceOnlineStatus(productKey, mac, true);
        //获取设备地理位置
        deviceService.locationDeviceAddress(productKey, mac);
