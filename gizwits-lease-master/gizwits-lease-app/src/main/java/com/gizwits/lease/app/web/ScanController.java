...
public class ScanController extends BaseController{
    ...
    @ApiOperation(value = "根据二维码内容搜索设备", consumes = "application/json")
    @PostMapping("/search")
    @ResponseBody
    public ResponseObject<DeviceForDetailDto> search(@RequestBody RequestObject<String> requestObject) {
        Device device = deviceService.getDeviceByQrcode(requestObject.getData());
        if (device == null) {
            LeaseException.throwSystemException(LeaseExceEnums.DEVICE_DONT_EXISTS);
        }
        return success(deviceService.detailForApp(device.getSno()));
    }
}
