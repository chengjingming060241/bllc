...
public class ProductServiceModeController extends BaseController {
    private final static Logger logger = LoggerFactory.getLogger("ORDER_LOGGER");
...
//        Integer userId = sysUser.getId();
        logger.info("sys_user_id"+current.getId()+",查询条件：收费模式名称："+requestObject.getData());
        ...
        if (pageable.getQuery().getSelfOperating()) {
            pageable.getQuery().setAccessableUserIds(Collections.singletonList(sysUserService.getCurrentUserOwner().getId()));
        } else {
            //查看所有上级的收费模式
            List<Integer> userIds = sysUserService.resolveAccessableUserIds(current, true);
...
//        pageable.getQuery().setCreatorId(userId);
    ...
    public ResponseObject<ProductServicecModeListDto> detail(@RequestBody @Valid RequestObject<Integer> requestObject) {
        ...
        return success(productServiceModeService.getProductServiceModeDetail(service_mode_id));
    }
