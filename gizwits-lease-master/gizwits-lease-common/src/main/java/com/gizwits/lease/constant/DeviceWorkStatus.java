...
public enum DeviceWorkStatus{
    ...
    public static Map enumToMap() {
        DeviceWorkStatus[] onlineStatuses = DeviceWorkStatus.class.getEnumConstants();
        Map map = new HashedMap();
        for (DeviceWorkStatus onlineStatus : onlineStatuses) {
            map.put(onlineStatus.getCode(), onlineStatus.getName());
        }
        return map;
    }

    public static DeviceWorkStatus getSDeviceWorkStatusEnum(int code) {
        for (DeviceWorkStatus deviceOnlineStatus : DeviceWorkStatus.values()) {
