...
//    private static final String UserInfoUrl = "https://api.weixin.qq.com/cgi-bin/user/info?access_token=ACCESS_TOKEN&openid=OPENID&lang=zh_CN";
    ...
    //客服消息-发送文本消息
    private static String SEND_TEXT_MESSAGE = "https://api.weixin.qq.com/cgi-bin/message/custom/send?access_token=ACCESS_TOKEN";
    /**
     * 微信设备
     */
    //二维码
    ...
    //获取卡券api_ticket
    private static final String GET_CARD_API_TICKET_URL = "https://api.weixin.qq.com/cgi-bin/ticket/getticket?access_token=ACCESS_TOKEN&type=wx_card";
    //批量查询卡券列表
    private static final String GET_CARD_LIST_URL = "https://api.weixin.qq.com/card/batchget?access_token=ACCESS_TOKEN";
    //查看卡券详情
    private static final String GET_CARD_DETAIL_URL = "https://api.weixin.qq.com/card/get?access_token=ACCESS_TOKEN";
    //获取用户已领取卡券接口
    private static final String GET_USER_CARD_LIST_URL = "https://api.weixin.qq.com/card/user/getcardlist?access_token=ACCESS_TOKEN";
    //查询卡券Code接口
    private static final String CHECK_CARD_CODE_URL = "https://api.weixin.qq.com/card/code/get?access_token=ACCESS_TOKEN";
    //核销卡券Code接口
    private static final String CONSUME_CARD_CODE_URL = "https://api.weixin.qq.com/card/code/consume?access_token=ACCESS_TOKEN";
    //修改卡券库存接口
    private static final String CARD_MODIFY_STOCK_URL = "https://api.weixin.qq.com/card/modifystock?access_token=ACCESS_TOKEN";
     ...
     * @param sysUserExt
     */
    public static String sendTextMessage(String body, SysUserExt sysUserExt) {
        return HttpUtil.executePost(SEND_TEXT_MESSAGE.replace("ACCESS_TOKEN", getAccessToken(sysUserExt)), body);
    }
    ...
    public static String getUserInfoByToken(String openid, String accessToken) {
        String url = UserInfoUrl_NEW.replace("OPENID", openid).replace("ACCESS_TOKEN", accessToken);
        System.out.println("====getUserInfo URL===:"+url);
        return HttpUtil.executeGet(url);
    }
    ...
    public static String getAccessToken(SysUserExt sysUserExt) {
        if (sysUserExt != null) {
            ...
            if (jsonObject.containsKey("errcode") && !jsonObject.get("errcode").equals("0")){
                LeaseException.throwSystemException(LeaseExceEnums.REQUEST_FAILTURE,jsonObject.getString("errmsg"));
            }
     ...
     * @param sysUserExt
     */
    public static String sendTemplateNotice(String body, SysUserExt sysUserExt) {
        return HttpUtil.executePost(SendTemplateUrl.replace("ACCESS_TOKEN", getAccessToken(sysUserExt)), body);
    }
     ...
     * @return
     */
    public static String customSendTextNews(String fromUserName, String content, SysUserExt sysUserExt) {
        String body = "{\"touser\":\"" + fromUserName + "\",\"msgtype\":\"text\""
    ...
    public static String postShareBenefitOrder(ShareBenefitSheet shareBenefitSheet, String ip, SysUserExt sysUserExt, Integer actionUserId) throws Exception {
        ...
        String resultStr = HttpUtil.httpPostWithCert(WXTransferURL, postDataXML, wxCertPath, sysUserExt.getWxParenterId());
    ...
    private static JSONObject handleWxResponse(String response, SysUserExt sysUserExt) {
        ...
        switch (errcode) {
            ...
            case 40014:
            case 42001:
                redisService.deleteAccessToken(sysUserExt.getWxId());
                break;
            default: break;
        }
        return jsonObject;
     ...
     * 获取卡券api_ticket
     */
    public static String getCardAPITicket(SysUserExt sysUserExt) {
        String cachedTicket = redisService.getWxCardAPITicket(sysUserExt.getWxAppid());
        if (cachedTicket != null) {
            logger.info("get cached wx card api ticket {} appid {}", cachedTicket, sysUserExt.getWxAppid());
            return cachedTicket;
        }

        logger.info("wx getCardAPITicket appid {}", sysUserExt.getWxAppid());
        String url = GET_CARD_API_TICKET_URL.replace("ACCESS_TOKEN", getAccessToken(sysUserExt));
        String response = HttpUtil.executeGet(url);
        logger.info("wx getCardAPITicket response {}", response);
        JSONObject jsonObject = handleWxResponse(response, sysUserExt);
        if (jsonObject.getInteger("errcode") == 0) {
            String ticket = jsonObject.getString("ticket");
            Long expires = jsonObject.getLong("expires_in");
            redisService.cacheWxCardAPITicket(sysUserExt.getWxAppid(), ticket, expires);
            logger.info("cache wx card api ticket {} appid {}", ticket, sysUserExt.getWxAppid());
            return ticket;
        }
        return null;
    }
     ...
     * 批量查询卡券列表
     */
    public static JSONObject getCardList(SysUserExt sysUserExt, Integer offset, Integer count, List<String> statusList) {
        logger.info("wx getCardList appid {} offset {} count {} statusList {}", sysUserExt.getWxAppid(), offset, count, statusList);
        String url = GET_CARD_LIST_URL.replace("ACCESS_TOKEN", getAccessToken(sysUserExt));
        JSONObject body = new JSONObject();
        body.put("offset", offset);
        body.put("count", count);
        body.put("status_list", statusList);
        String response = HttpUtil.executePost(url, body.toJSONString());
        logger.info("wx getCardList response {}", response);
        return handleWxResponse(response, sysUserExt);
    }

    /**
     * 批量查询卡券列表
     */
    public static JSONObject getCardDetail(SysUserExt sysUserExt, String cardId) {
        logger.info("wx getCardDetail appid {} cardId {}", sysUserExt.getWxAppid(), cardId);
        String url = GET_CARD_DETAIL_URL.replace("ACCESS_TOKEN", getAccessToken(sysUserExt));
        JSONObject body = new JSONObject();
        body.put("card_id", cardId);
        String response = HttpUtil.executePost(url, body.toJSONString());
        logger.info("wx getCardDetail response {}", response);
        return handleWxResponse(response, sysUserExt);
    }
     ...
     * 获取用户已领取卡券接口
     */
    public static JSONObject getUserCardList(SysUserExt sysUserExt, String openid) {
        logger.info("wx getUserCardList openid {} appid {}", openid, sysUserExt.getWxAppid());
        String url = GET_USER_CARD_LIST_URL.replace("ACCESS_TOKEN", getAccessToken(sysUserExt));
        JSONObject body = new JSONObject();
        body.put("openid", openid);
        String response = HttpUtil.executePost(url, body.toJSONString());
        logger.info("wx getUserCardList response {}", response);
        return handleWxResponse(response, sysUserExt);
    }
     ...
     * 查询卡券Code接口
     */
    public static JSONObject checkCardCode(SysUserExt sysUserExt, String cardId, String code) {
        logger.info("wx checkCardCode cardId {} code {} appid {}", cardId, code, sysUserExt.getWxAppid());
        String url = CHECK_CARD_CODE_URL.replace("ACCESS_TOKEN", getAccessToken(sysUserExt));
        JSONObject body = new JSONObject();
        body.put("code", code);
        body.put("card_id", cardId);
        body.put("check_consume", true);
        String response = HttpUtil.executePost(url, body.toJSONString());
        logger.info("wx checkCardCode response {}", response);
        return handleWxResponse(response, sysUserExt);
    }
     ...
     * 核销卡券Code接口
     */
    public static JSONObject consumeCardCode(SysUserExt sysUserExt, String cardId, String code) {
        logger.info("wx consumeCardCode cardId {} code {} appid {}", cardId, code, sysUserExt.getWxAppid());
        String url = CONSUME_CARD_CODE_URL.replace("ACCESS_TOKEN", getAccessToken(sysUserExt));
        JSONObject body = new JSONObject();
        body.put("code", code);
        body.put("card_id", cardId);
        String response = HttpUtil.executePost(url, body.toJSONString());
        logger.info("wx consumeCardCode response {}", response);
        return handleWxResponse(response, sysUserExt);
    }
     ...
     * 修改卡券库存接口
     */
    public static JSONObject modifyCardStock(SysUserExt sysUserExt, String cardId, int reduce) {
        logger.info("wx modifyCardStock cardId {} reduce {} appid {}", cardId, reduce, sysUserExt.getWxAppid());
        String url = CARD_MODIFY_STOCK_URL.replace("ACCESS_TOKEN", getAccessToken(sysUserExt));
        JSONObject body = new JSONObject();
        body.put("card_id", cardId);
        body.put("reduce_stock_value", reduce);
        String response = HttpUtil.executePost(url, body.toJSONString());
        logger.info("wx modifyCardStock response {}", response);
        return handleWxResponse(response, sysUserExt);
    }
}