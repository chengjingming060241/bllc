...
public class DeviceControlAPI {
    ...
    public static DeviceAddressModel getDeviceAddress(String productKey, String did) {
        String productInfo = redisService.getProductData(productKey);
        if (ParamUtil.isNullOrEmptyOrZero(productInfo)){
            logger.error("===> 系统中不存在productKey[" + productKey + "]的产品信息");
            return null;
        }
        Product productEntity = JSONObject.parseObject(productInfo, Product.class);
        if (productEntity == null) {
            logger.error("===> 系统中不存在productKey[" + productKey + "]的产品信息");
            return null;
        }
