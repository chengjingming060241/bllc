...
public class ManufacturerCommonRoleResolver extends AbstractCommonRoleResolver {
    ...
    public List<AssignDestinationDto> resolveAssignDest() {
        //运营商和代理商
        return Stream.of(AssignDestinationType.AGENT).map(AssignDestinationDto::new).collect(Collectors.toList());
    }
  ...
  /*  @Override
    protected boolean doAssign(DeviceForAssignDto dto, SysUserRoleTypeHelper helper) {
        ...
        } else if (Objects.equals(dto.getAssignDestinationType(), AssignDestinationType.OPERATOR.getCode()) || Objects.equals(dto.getAssignDestinationType(), AssignDestinationType.AGENT.getCode()) ) {
